...
const RecentActivity = ({
  ...
  return <div className="p-4 bg-slate-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        {activities.map(activity => <div key={activity.id} className={`group p-3 rounded-xl border border-white/20 bg-white/10 backdrop-blur-sm hover:border-white/30 hover:bg-white/20 transition-all duration-300 ${activity.type === 'document' ? 'cursor-pointer hover:scale-[1.02]' : ''}`} onClick={() => handleActivityClick(activity)}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between">
                  <Badge variant="secondary" className="text-xs px-2 py-1 bg-white/20 text-white border-0 font-medium rounded-lg backdrop-blur-sm">
                    {activity.user}
                  </Badge>
