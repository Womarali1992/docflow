...
import { X, ChevronDown, Calendar, Clock, AlertCircle, CheckCircle2, CalendarDays } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { useDocumentsStore } from '@/context/DocumentsContext';
import { RequestFrequency, DocumentTimePeriod } from '@/types/dashboard';
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from '@/components/ui/collapsible';
import { Separator } from '@/components/ui/separator';
import TimePeriodSelector from './TimePeriodSelector';
import { inferFrequencyFromLabel, getFrequencyColor, formatTimePeriod, groupTimePeriodsByYear } from '@/utils/documentUtils';
import { DOCUMENT_TYPES, UI_LIMITS } from '@/constants/app';
...
const getFrequencyIcon = (frequency: RequestFrequency) => {
  switch (frequency) {
    case 'monthly':
...
const DocumentsNeeded = ({ clientId, advisorName }: DocumentsNeededProps) => {
  ...
  const { requestDocument, updateDocumentTimePeriods } = useDocumentsStore();
  ...
  const handleDocumentClick = (item: BinItem, binId: string) => {
    const frequency = inferFrequencyFromLabel(bins.find(b => b.id === binId)?.label || '');
    
    // Only show time period selector for recurring documents
    if (frequency === 'monthly' || frequency === 'quarterly' || frequency === 'yearly') {
      setSelectedDocument({ item, binId, frequency });
      setTimeSelectorOpen(true);
    }
  };

  const handleTimePeriodsSave = (selectedPeriods: string[]) => {
    if (!selectedDocument) return;

    const { item, frequency } = selectedDocument;
    
    // Create time period objects
    const timePeriods: DocumentTimePeriod[] = selectedPeriods.map(period => ({
      id: `${item.id}-${period}`,
      documentId: item.id,
      period,
    ...
    // Update the bin item with selected time periods
    setBins(prev => prev.map(bin => 
      bin.id === selectedDocument.binId
        ? {
            ...bin,
            items: bin.items.map(i => 
              i.id === item.id 
                ? { ...i, selectedTimePeriods: timePeriods }
                : i
            )
          }
        : bin
    ...
    updateDocumentTimePeriods(item.id, timePeriods);

    toast({
      title: 'Time Periods Updated',
      description: `${selectedPeriods.length} time period${selectedPeriods.length !== 1 ? 's' : ''} selected for ${item.name}.`,
    });

    setSelectedDocument(null);
  };

  const getTimePeriodsDisplay = (item: BinItem) => {
    if (!item.selectedTimePeriods || item.selectedTimePeriods.length === 0) {
      return null;
    }

    const frequency = item.selectedTimePeriods[0]?.periodType;
    const periods = item.selectedTimePeriods.map(tp => tp.period);
    
    const periodsByYear = groupTimePeriodsByYear(item.selectedTimePeriods);



    return (
      <div className="space-y-2 mt-2">
        {Object.entries(periodsByYear).map(([year, yearTimePeriods]) => (
          <div key={year} className="space-y-1">
            <div className="text-xs font-medium text-gray-600">{year}</div>
            <div className="flex flex-wrap gap-1">
              {yearTimePeriods.slice(0, UI_LIMITS.MAX_DISPLAYED_PERIODS).map(timePeriod => (
                <Badge
                  key={timePeriod.id}
                  variant="outline"
                  className="text-xs px-2 py-1 bg-blue-50 text-blue-700 border-blue-200"
                >
                  {formatTimePeriod(timePeriod.period, frequency)}
                </Badge>
              ))}
              {yearTimePeriods.length > UI_LIMITS.MAX_DISPLAYED_PERIODS && (
                <Badge
                  variant="outline"
                  className="text-xs px-2 py-1 bg-gray-50 text-gray-600 border-gray-200"
                >
                  +{yearTimePeriods.length - UI_LIMITS.MAX_DISPLAYED_PERIODS} more
                </Badge>
              )}
            </div>
          </div>
    ...
    <>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {bins.map((bin) => {
                    ...
                    return (
                      <div
                        key={bin.id}
                        className={`group relative rounded-xl p-3 sm:p-6 min-h-[120px] sm:min-h-[160px] border-2 border-dashed transition-all duration-300 ${
                          bin.isDragOver 
                        ...
                        <div className="space-y-3">
                          {bin.items.length === 0 ? (
                            ...
                            bin.items.map((item) => (
                              <div 
                                key={item.id} 
                                className={`group/item flex flex-col gap-2 p-3 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-all duration-200 ${
                                  (frequency === 'monthly' || frequency === 'quarterly' || frequency === 'yearly') 
                                    ? 'cursor-pointer hover:border-blue-300 hover:bg-blue-50/30' 
                                    : ''
                                }`}
                                onClick={() => handleDocumentClick(item, bin.id)}
                              >
                                ...
                                {getTimePeriodsDisplay(item)}
                                
                                {/* Click Hint for Recurring Documents */}
                                {(frequency === 'monthly' || frequency === 'quarterly' || frequency === 'yearly') && (
                                  <div className="text-xs text-blue-600 font-medium">
                                    Click to select time periods
                                  </div>
                                )}
                              </div>
                            ))
      ...
      {selectedDocument && (
        <TimePeriodSelector
          ...
          documentName={selectedDocument.item.name}
          frequency={selectedDocument.frequency}
          onSave={handleTimePeriodsSave}
          initialSelected={selectedDocument.item.selectedTimePeriods?.map(tp => tp.period) || []}
        />
      )}
    </>
  );
