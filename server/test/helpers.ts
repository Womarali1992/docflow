 ...
 *
 * It lived in `uploads.test.ts` until the hardening release needed it too: the
 * out-of-order publication tests (F4) only mean anything against a scanner that
 * answers, and two copies of a protocol stub is one too many.
 */
export function fakeClamd(behaviour: 'clean' | 'infected' | 'garbage' | 'silent' | 'pong') {
